...
- A family or ordered genome run writes `family_multipliers.tsv`, with one row per family and the
  ...
  `transposition` and `translocation` columns. (#442)
...
- A joint run on a tree passed with `tree=` reads a declared family's own duplication, transfer and
...
- Ordered genomes take joint runs: with `joint=True` a rate or an extent can read
  ...
  a per-family draw does. The run writes `links.tsv`, as a family run does. (#431)
...
- **BREAKING:** a genome result's `.genomes` is now the observed dataset — the genome at each extant
...
- **A nucleotide genome can be a driver.** `NucleotideGenomesResult` gains `.presence(name)` and
  ...
  named group of declared genes, as at the other two resolutions, for `.completion()` to read. (#322)
- **A sequence can be a driver: its composition.** `SequencesResult.composition(letters)` gives the
  ...
  run on its own output (SPEC §3, a pair that can only be joined). (#322)
- **`SequencesResult.alphabet`** — what a run's sequences are written in (`"ACGT"`, or the 20 amino
  acids). One per run, and what `composition` checks its letters against. (#322)
- **`zombi2 traits --switch` takes a rate in its written form**, as `--rate` already did:
  ...
  which names the model and the way out. (#322)
...
- **A driver inside a `--switch` dict is no longer invisible to the run's record.** The parameters
  ...
  a conditioned run cannot read as an unconditioned one, nor have its driver re-run out from under it. (#322)
