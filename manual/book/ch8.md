...
![The three kinds of run. Left: two independent runs, one after the other. Middle: a conditioned run, where the second run reads the results of the first. Right: a joint run, where both levels are simulated at the same time.](figures/execution_print.png){width=95%}
...
![A connection. The driver is the value that is read. The target is the parameter that depends on it. The link states how each value of the driver becomes a factor on the target.](figures/connection_abstract_print.png){width=80%}

A connection has three parts. The **driver** is the value that is read: the state of a trait, the presence of a gene family, the GC content of a gene. The **target** is the parameter that depends on it: a loss rate, a substitution rate, a speciation rate. The **link** is what joins them: it states what number each value of the driver produces, and what that number does to the target.
...
| a trait and a gene's sequence | handed | slices; the composition is read at the start of the slice |
