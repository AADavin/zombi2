...
A continuous trait is recorded only at the nodes, so what it did between two of them has to come from somewhere. ZOMBI2 draws it: the trait's path along the branch, conditioned on the value at each end. The path is drawn after the trait is simulated, from its own stream, so the trait's node values are the same numbers whether or not anything reads the path. Write `path=False` on the link to read the straight line between the two node values instead. The line is that path's average, and averaging first is not the same as reading a curve: under a mapping like `exp`, the line gives a rate that is too low, and halving `step` does not fix it because the line does not move. A run writes its path to `trait_path.tsv`, so a driver read from a file gives the same answer as the same driver held in memory.
