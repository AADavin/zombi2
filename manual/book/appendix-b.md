...
| Conditioning | `conditioned_on` | text | conditioned | as at the family resolution: written **only when a rate or `transfer_to` was conditioned** — the run's levels this run read via `DrivenBy`, one per line |
| Tip names | `names.tsv` | TSV | external tree | as at the family resolution |
...
| Output | File | Format | Default | Contents |
|-----------|-----------------|-------|-----|------------------------|
| Species tree | `species/species_complete.nwk` · `…_extant.nwk` · `species_events.tsv` · `species_fates.tsv` | Newick, TSV | yes | the grown tree — complete, so the extinct lineages whose fate the driver decided are kept |
...
| Summary | `joint_summary.json` | JSON | yes | both levels' summaries in one file — `seed` · `driver` (`trait` / `genome`) · `species`, whose realised rates are what the driver did · one of `trait` / `genome`, the driver's own. It sits at the **run root** rather than under a level, because neither level was grown first. The same payloads the two levels write alone, so there is nothing new to read |
