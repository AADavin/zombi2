...
- **Families need not evolve at the same pace.** `ByFamily` draws a factor per family (`--loss "0.25 * ByFamily(spread=0.6)"`). Whether you write one draw or two decides what varies together: two separately built `ByFamily` draws vary their rates independently, while **one** object read by several rates gives that family a single tempo across all of them — `speed = mod.ByFamily(spread=0.5)`, then `duplication=0.2 * speed, loss=0.25 * speed`.
