...
The optimum can also follow another continuous trait, grown first on the same tree. `reverts_to=set_by(group_size, lambda v: 1.0 + 0.8 * v, step=0.01)` makes a larger group keep pulling the trait upward. The function turns the other trait's value into the optimum, and the optimum can be any number, negative included. ZOMBI2 knows the other trait only at the nodes, so along a branch it reads the straight line between them, in stretches of at most `step`. Halve `step` and rerun to check the result does not move. The effect runs one way: the other trait steers this one, and this one does not steer it back.
