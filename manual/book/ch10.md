...
Growing the trait first would need a tree that does not exist yet; growing the tree first would need the trait. Neither can go first, so **one run simulates both**. That is what joining is. The test is one question: *can the driver be grown on its own and handed over?* If it can, condition. If it cannot, **join** ([Jo4, Jo7](https://aadavin.github.io/zombi2/gallery.html#joining)<!--gallery:key_innovation--><!--gallery:musse-->).
...
![What can be joined. Every arrow has two heads, because neither level can be finished before the other starts: a joint pair still has a driver and a target, but no order to simulate them in. Compare the figure at the head of this chapter, and the conditioning map in Chapter 9, where the arrows run one way. Solid arrows are the five joint models ZOMBI2 has. A dashed arrow is a pair nothing rules out and nobody has written; a loop is a level joined to itself. One pair has no arrow at all.](figures/joining_map_print.png){width=88%}

Five are built, and they split in two. In the two that reach **Species**, the tree is one of the levels being simulated, so it comes out of the run. In the other three the tree is handed to the run like any other input: **Genomes with Traits**, where gene content and a character set each other's rates; **Genomes with itself**, where one named family sets the rates of the genome it sits in; and **Traits with itself**, two characters each reading the other.

The three dashed ones are empty. Nothing rules them out, and the nearest to reach is a continuous trait driving speciation, which needs the rate to be held still over short stretches rather than read at events.
...
A trait and a sequence are a pair, and the reason is that both directions exist. A trait drives a gene's substitution rate, and that gene's composition drives how fast a trait changes ([Co8, Co18](https://aadavin.github.io/zombi2/gallery.html#conditioning)<!--gallery:climate_substitution--><!--gallery:gc_drives_trait-->). Either one alone is conditioning, because the driver can be finished first. Write both at once and there is a cycle, and a cycle has no order to simulate in.
...
|---|---|---|
| `"traits:<name>"` | that trait's current state | a table over the states |
...
The level itself is given as a **process spec** — `traits.discrete(...)` or `genomes.genome(...)` — a description of a process still to be grown rather than a finished result. A spec keeps all of its own options, and one of them changes the model: `at_speciation=` (Chapter 8) gives the trait a chance of jumping *at* each split, so the daughters diverge at the moment they are born while how fast a lineage splits still depends on the state it is in. The trait's event log tells the two apart, `on_speciation` against `on_branch`.
...
`zombi2 joint` covers the two models above. The rate is written exactly as in Python, and the flags that build the second level are the ones `zombi2 traits` or `zombi2 genomes` would take. Giving flags from both is an error rather than a silent choice between them.
...
**These two are the whole of the command.** The two models in the next section are Python only, and deliberately: one needs a rate written for a single named family, which no flag can carry, and the other needs two levels' worth of flags plus a tree to run them on. A command line is for the runs you write often enough to want short, and these are not those yet.
...
A cave costs a lineage its genes; losing the eye commits it further to the cave. Neither can be simulated first, because to grow the habitat you need to know whether the eye is still there, and whether the eye survives depends on the habitat ([Jo2](https://aadavin.github.io/zombi2/gallery.html#joining)<!--gallery:cave_genomes-->).
...
One trait grown first and then read is conditioning, and Chapter 9 covers it. This is the case with no order — body size sets how readily a lineage goes underground, and living underground sets how readily it grows ([Jo1](https://aadavin.github.io/zombi2/gallery.html#joining)<!--gallery:trait_loop-->).
...
|-------------------|--------------------------------|---|
| a binary trait drives speciation (and extinction) | BiSSE [@maddison2007bisse] | [Jo5](https://aadavin.github.io/zombi2/gallery.html#joining)<!--gallery:bisse--> |
| a multi-state trait drives speciation | MuSSE [@fitzjohn2012diversitree] | [Jo7](https://aadavin.github.io/zombi2/gallery.html#joining)<!--gallery:musse--> |
| a trait drives speciation **and** jumps at the split | ClaSSE [@goldberg2012classe] | [Jo8](https://aadavin.github.io/zombi2/gallery.html#joining)<!--gallery:classe--> |
