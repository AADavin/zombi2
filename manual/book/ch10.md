...
Growing the trait first would need a tree that does not exist yet; growing the tree first would need the trait. Neither can go first, so **one run simulates both**. That is what joining is. The test is one question: *can the driver be grown on its own and handed over?* If it can, condition. If it cannot, **join** ([Jo5, Jo8](https://aadavin.github.io/zombi2/gallery.html#joining)<!--gallery:key_innovation--><!--gallery:musse-->).
...
![What can be joined. Every arrow has two heads, because neither level can be finished before the other starts: a joint pair still has a driver and a target, but no order to simulate them in. Compare the figure at the head of this chapter, and the conditioning map in Chapter 9, where the arrows run one way. Solid arrows are the six joint models ZOMBI2 has. A dashed arrow is a pair nothing rules out and nobody has written; a loop is a level joined to itself. One pair has no arrow at all.](figures/joining_map_print.png){width=88%}

Six are built, and they split in two. In the two that reach **Species**, the tree is one of the levels being simulated, so it comes out of the run. In the other four the tree is handed to the run like any other input: **Genomes with Traits**, where gene content and a character set each other's rates; **Genomes with itself**, where one named family sets the rates of the genome it sits in; **Traits with itself**, two characters each reading the other; and **Sequences with itself**, two genes each reading how much of the other is a given set of letters.

The two dashed ones are empty. Nothing rules them out, and both need the same thing the sequence loop needed: a rate held still over short stretches rather than read at events, because what drives it is a number that moves continuously. That method is built twice over now, so what these two need is the work of pointing it at a new pair rather than a new idea.
...
A trait and a sequence are a pair, and the reason is that both directions exist. A trait drives a gene's substitution rate, and that gene's composition drives how fast a trait changes ([Co8, Co19](https://aadavin.github.io/zombi2/gallery.html#conditioning)<!--gallery:climate_substitution--><!--gallery:gc_drives_trait-->). Either one alone is conditioning, because the driver can be finished first. Write both at once and there is a cycle, and a cycle has no order to simulate in.
...
The level itself is given as a **process spec** — `traits.discrete(...)`, `traits.continuous(...)`, `genomes.genome(...)` or `sequences.gene(...)` — a description of a process still to be grown rather than a finished result. A spec keeps all of its own options, and one of them changes the model: `at_speciation=` (Chapter 8) gives the trait a chance of jumping *at* each split, so the daughters diverge at the moment they are born while how fast a lineage splits still depends on the state it is in. The trait's event log tells the two apart, `on_speciation` against `on_branch`.
...
What comes back is a `JointResult` carrying **both** simulated levels. `.species` is always there, and then `.trait`, `.genome`, or both. They are the same result objects the standalone functions return, they share one `complete_tree` because there was only ever one tree, and the run writes both levels exactly as their own commands would.
...
The two above race exactly. This one does not, and it is the only model in the chapter that does not.
...
The two models above simulate the species tree. The other two take one, and are written the same way — participants, minus the species spec, plus `tree=`.

### A trait and a genome, each other's driver

A cave costs a lineage its genes; losing the eye commits it further to the cave. Neither can be simulated first, because to grow the habitat you need to know whether the eye is still there, and whether the eye survives depends on the habitat ([Jo3](https://aadavin.github.io/zombi2/gallery.html#joining)<!--gallery:cave_genomes-->).
...
One trait grown first and then read is conditioning, and Chapter 9 covers it. This is the case with no order — body size sets how readily a lineage goes underground, and living underground sets how readily it grows ([Jo2](https://aadavin.github.io/zombi2/gallery.html#joining)<!--gallery:trait_loop-->).
...
# LG's chemistry over KR-depleted frequencies: where the two genes start from
m = lg()
...
|-------------------|--------------------------------|---|
| a binary trait drives speciation (and extinction) | BiSSE [@maddison2007bisse] | [Jo6](https://aadavin.github.io/zombi2/gallery.html#joining)<!--gallery:bisse--> |
| a multi-state trait drives speciation | MuSSE [@fitzjohn2012diversitree] | [Jo8](https://aadavin.github.io/zombi2/gallery.html#joining)<!--gallery:musse--> |
| a trait drives speciation **and** jumps at the split | ClaSSE [@goldberg2012classe] | [Jo9](https://aadavin.github.io/zombi2/gallery.html#joining)<!--gallery:classe--> |
| a **continuous** trait drives speciation | QuaSSE [@fitzjohn2010quasse] | [Jo10](https://aadavin.github.io/zombi2/gallery.html#joining)<!--gallery:quasse--> |
