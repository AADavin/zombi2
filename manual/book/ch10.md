...
The live `source` names either the family, `"genomes:toxin"`, or the lineage's total gene count, `"genomes:count"` — and a count is a number, so that is where a **Curve** is used instead of a table: `mod.DrivenBy("genomes:count", lambda n: math.exp(0.02 * n))` makes gene-rich lineages speciate faster along a smooth response.
