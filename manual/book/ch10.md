...
Growing the trait first would need a tree that does not exist yet; growing the tree first would need the trait. Neither can go first, so **one run simulates both**. That is what joining is. The test is one question: *can the driver be grown on its own and handed over?* If it can, condition. If it cannot, **join** ([Jo6, Jo9](https://aadavin.github.io/zombi2/gallery.html#joining)<!--gallery:key_innovation--><!--gallery:musse-->).
...
![What can be joined. Every arrow has two heads, because neither level can be finished before the other starts: a joint pair still has a driver and a target, but no order to simulate them in. Compare the figure at the head of this chapter, and the conditioning map in Chapter 9, where the arrows run one way. Solid arrows are the seven joint models ZOMBI2 has. The dashed arrow is the pair nothing rules out and nobody has written; a loop is a level joined to itself. One pair has no arrow at all.](figures/joining_map_print.png){width=88%}

Seven are built, and they split in two. In the two that reach **Species**, the tree is one of the levels being simulated, so it comes out of the run. In the other five the tree is handed to the run like any other input: **Genomes with Traits**, where gene content and a character set each other's rates; **Traits with Sequences**, where a character sets a gene's substitution rate and that gene's composition sets how fast the character switches; **Genomes with itself**, where one named family sets the rates of the genome it sits in; **Traits with itself**, two characters each reading the other; and **Sequences with itself**, two genes each reading how much of the other is a given set of letters.

One is dashed and empty. Nothing rules out a genome and a sequence being joined — only conditioning is ruled out there, because a sequence is grown along the gene trees the genome level produced, so reading it back would condition a run on its own output.
...
A cave costs a lineage its genes; losing the eye commits it further to the cave. Neither can be simulated first, because to grow the habitat you need to know whether the eye is still there, and whether the eye survives depends on the habitat ([Jo4](https://aadavin.github.io/zombi2/gallery.html#joining)<!--gallery:cave_genomes-->).
...
One trait grown first and then read is conditioning, and Chapter 9 covers it. This is the case with no order — body size sets how readily a lineage goes underground, and living underground sets how readily it grows ([Jo3](https://aadavin.github.io/zombi2/gallery.html#joining)<!--gallery:trait_loop-->).
...
|-------------------|--------------------------------|---|
| a binary trait drives speciation (and extinction) | BiSSE [@maddison2007bisse] | [Jo7](https://aadavin.github.io/zombi2/gallery.html#joining)<!--gallery:bisse--> |
| a multi-state trait drives speciation | MuSSE [@fitzjohn2012diversitree] | [Jo9](https://aadavin.github.io/zombi2/gallery.html#joining)<!--gallery:musse--> |
| a trait drives speciation **and** jumps at the split | ClaSSE [@goldberg2012classe] | [Jo10](https://aadavin.github.io/zombi2/gallery.html#joining)<!--gallery:classe--> |
| a **continuous** trait drives speciation | QuaSSE [@fitzjohn2010quasse] | [Jo11](https://aadavin.github.io/zombi2/gallery.html#joining)<!--gallery:quasse--> |
