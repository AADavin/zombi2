...
def test_transfer_neighbors_respect_time_overlap():
    ...
    from zombi2.tools.reconciliation.undated import _transfer_neighbors
    sp = SpeciesTree.from_tree(read_newick("((A:1,B:1)i1:1,(C:1,D:1)i2:1)root;"))
...
def test_undated_reldated_rust_matches_python():
    ...
    from zombi2.tools.reconciliation import _rust
...
# ============================================================ zombi2.tools public surface

def test_reconciliation_likelihood_wrapper_matches_engines():
    """The one-call `reconciliation_likelihood` reproduces the underlying engine functions for
    every model, from both a Reconciliation and an explicit gene tree."""
    tree = simulate_species_tree(Yule(2.0), n_tips=6, age=1.0, seed=9)
    g = simulate_genomes(tree, duplication=0.2, transfer=0.1, loss=0.3,
                         origination=0.0, initial_families=10, seed=9)
    sp = SpeciesTree.from_tree(tree)
    recon = next(r for r in g.reconciliations().values() if r.extant is not None)
    gt = GeneTree.from_reconciliation(recon)
    d, t, lo = 0.2, 0.1, 0.3
    expected = {
        "dated": dated_loglik(gt, sp, DatedDTL(d, t, lo), origination="root", n_steps=50),
        "undated": undated_loglik(gt, sp, UndatedDTL(d, t, lo), origination="root"),
        "reldated": reldated_loglik(gt, sp, UndatedDTL(d, t, lo), origination="root"),
    }
    for model, exp in expected.items():
        got = reconciliation_likelihood(recon, tree, duplication=d, transfer=t, loss=lo,
                                        model=model, n_steps=50)
        assert math.isclose(got, exp, rel_tol=1e-9, abs_tol=1e-9), model
        # same answer via an explicit extant-Newick gene tree
        got_nwk = reconciliation_likelihood(gene_tree=recon.extant, species_tree=tree,
                                            duplication=d, transfer=t, loss=lo,
                                            model=model, n_steps=50)
        assert math.isclose(got_nwk, exp, rel_tol=1e-9, abs_tol=1e-9), model


def test_reconciliation_likelihood_argument_validation():
    """`reconciliation_likelihood` rejects bad model names and ambiguous / missing inputs."""
    tree = simulate_species_tree(Yule(2.0), n_tips=5, age=1.0, seed=2)
    with pytest.raises(ValueError, match="unknown model"):
        reconciliation_likelihood(gene_tree="A|1;", species_tree=tree, model="bogus")
    with pytest.raises(ValueError, match="exactly one"):
        reconciliation_likelihood(species_tree=tree)  # neither reconciliation nor gene_tree
    with pytest.raises(ValueError, match="species_tree is required"):
        reconciliation_likelihood(gene_tree="A|1;")


def test_reconciliation_likelihood_class_reuses_background():
    """The ReconciliationLikelihood class scores each family exactly as the function does, and
    `score_all` matches per-family `score`."""
    tree = simulate_species_tree(Yule(2.0), n_tips=6, age=1.0, seed=13)
    g = simulate_genomes(tree, duplication=0.15, transfer=0.08, loss=0.25,
                         origination=0.0, initial_families=12, seed=13)
    scorer = ReconciliationLikelihood(tree, duplication=0.15, transfer=0.08, loss=0.25,
                                      model="dated", n_steps=50)
    recons = g.reconciliations()
    rows = {r.family: r for r in scorer.score_all(recons)}
    checked = 0
    for fam, recon in recons.items():
        if recon.extant is None:
            continue
        via_class = scorer.score(recon)
        via_func = reconciliation_likelihood(recon, tree, duplication=0.15, transfer=0.08,
                                             loss=0.25, model="dated", n_steps=50)
        assert math.isclose(via_class, via_func, rel_tol=1e-12)
        assert math.isclose(rows[fam].logliks["dated"], via_class, rel_tol=1e-12)
        checked += 1
    assert checked > 0


def test_tools_are_not_reexported_into_top_level():
    """zombi2.tools is a distinct, labelled surface: nothing here leaks into top-level zombi2."""
    import zombi2
    import zombi2.tools

    assert hasattr(zombi2.tools, "reconciliation_likelihood")
    assert hasattr(zombi2.tools, "ReconciliationLikelihood")
    for name in ("reconciliation_likelihood", "ReconciliationLikelihood", "UndatedDTL",
                 "DatedDTL", "score_reconciliations"):
        assert not hasattr(zombi2, name), f"{name} leaked into top-level zombi2"
