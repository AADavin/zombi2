...
def score_reconciliations(species_tree, reconciliations, dup: float, transfer: float, loss: float,
    ...
    ``species_tree`` is a :class:`zombi2.tree.Tree` (the extant/reconstructed species tree) or an
    already-built :class:`SpeciesTree`; ``reconciliations`` is ``{family: Reconciliation}`` (from
    ``Genomes.reconciliations()``). Families with no surviving copy are skipped (no observable
    gene tree). Returns a list of :class:`FamilyScore` in reconciliation order.
    """
