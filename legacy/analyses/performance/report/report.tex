...
(\SI{4.9}{\second}, $\approx$\num{3000000} events). The \emph{event trace}
(\texttt{output="trace"}) records the same D/T/L/O history but drops the speciation
``carry'' events entirely --- a gene keeps its id across speciations, and the implied
bifurcations are recovered by replaying the trace against the species tree --- so gene
trees remain reconstructable on demand while the cost falls to barely above counts-only:
\SI{1.4}{\second} at \num{100000} tips and \SI{19}{\second} at \num{1000000}. The
...
the millions: \SI{63}{\milli\second} at \num{10000} tips, \SI{1.1}{\second} at
\num{100000}, and \SI{16}{\second} at \num{1000000} tips.
...
\emph{event trace} sits just above it: by dropping the redundant speciation ``carry''
rows (recoverable from the species tree) and keeping only per-family leaf counts, it
holds the whole D/T/L/O history for a million tips in \SI{5.1}{GB} --- close to the
counts-only floor, and far below the full log's \SI{3.5}{GB} already at \num{100000}
tips (one Python object per event). The residual gap to the profile is the trace
columns themselves, held as Python lists.
...
\begin{figure}[h]
  ...
  \caption{Peak resident memory versus tip count (isolated subprocess per size).
    The species tree, the sparse profiles, and the event trace all scale linearly into
    the millions (the trace just above the counts-only floor); the full event log is the
    memory-heaviest path and caps near \num{100000} tips.}
  \label{fig:mem}
\end{figure}
...
\subsection{A stricter test: both engines on one shared tree}
\label{sec:vs-fixed}

The comparison above grows each tool's tree in its own native way, so the two trees are
not identical: under a pure-birth Yule process ($\lambda{=}1$) the tree height grows as
$\ln N$, whereas ZOMBI2's default regime fixes the age, so at a matched tip count
ZOMBI~1's tree is the deeper of the two (see \texttt{vs\_zombi1/NOTES.md}). To remove
this last confound we ran a fully controlled test: a \emph{single} species tree, with
\emph{both} engines' genome step run over that identical tree.

We build one pure-birth ZOMBI~1 tree of \num{1000} extant tips (no extinction, so the
complete tree \emph{is} the extant tree; age~$5.0$). ZOMBI2 reads it via
\texttt{read\_newick}; ZOMBI~1 runs its \texttt{Gm} step on the same folder. Both use the
identical regime ($D{=}0.2$, $T{=}0.1$, $L{=}0.25$, $O{=}0.5$; \num{20} founding families;
size-neutral replacement transfers) and \num{10} independent seeds, one timed run each ---
so the topology, the branch-length/time integral and the rates are all shared, and the
\emph{only} variable is the engine. The two engines produce comparable family counts
(\num{464} versus \num{524}), confirming a like-for-like workload.

On this identical tree ZOMBI2 completes the genome simulation in a median of
\SI{70.6}{\milli\second} against ZOMBI~1's \SI{41.0}{\second} ---
\headline{$\approx\!581\times$ faster} (Figure~\ref{fig:vs-fixed}). Because the shared
tree is deeper than ZOMBI2's native age-2 regime, ZOMBI2 here does roughly twice its
native-regime work ($\approx$\SI{38}{\milli\second}, \S\ref{sec:vs}); the two measurements
therefore \emph{bracket} the speed-up --- a conservative $\approx\!581\times$ under a
strictly identical tree, and $>\!1000\times$ when each tool uses its own native regime.
We report both.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.58\linewidth]{vs_zombi1_fixedtree.png}
  \caption{Per-run wall-clock of the genome step for both engines on \emph{one shared}
    \num{1000}-tip species tree (\num{10} seeds each; log scale, markers are individual
    runs). Fixing the tree removes the tree-depth confound of Figure~\ref{fig:vs};
    ZOMBI2 is $\approx\!581\times$ faster in the median. Opt-in benchmark
    (\texttt{vs\_zombi1\_fixedtree}).}
  \label{fig:vs-fixed}
\end{figure}
...
\begin{table}[h]
  ...
  \begin{tabular}{l S[table-format=7.0] S[table-format=5.1] l l}
    ...
    Gene families, full log   & 100000  & 4.9  & s & \SI{3.5}{GB}  \\
    Gene families, event trace& 100000  & 1.4  & s & \SI{0.64}{GB} \\
    Gene families, event trace& 1000000 & 19.0 & s & \SI{5.1}{GB}  \\
    Gene families, profiles   & 1000000 & 16.0 & s & \SI{3.2}{GB}  \\
    \bottomrule
  \end{tabular}
\end{table}
...
linear. Of the three gene-family outputs, the counts-only profile is the cheapest, the
event trace keeps gene trees reconstructable for barely more (both reach a million tips
at close to the same time and memory), and the full per-event log --- the richest but
heaviest --- is best kept to $\sim$\num{100000} tips. Against the legacy
...
\begin{figure}[h]
  ...
  \caption{Overview: (a) species-tree scaling, (b) genome (unordered) simulation
    --- full gene trees versus counts-only profiles, (c) ZOMBI2 versus the legacy
    ZOMBI~1 on one shared 1{,}000-tip species tree (per-run wall-clock, ten runs each).}
  \label{fig:overview}
\end{figure}

\vfill
{\small\color{muted}
Reproducible from \texttt{analyses/performance/}: \texttt{python run.py} writes raw
