...
configuration is reproducible and shareable — the modern successor to ZOMBI1's
``*Parameters.tsv``. It is applied as **defaults**, so any flag given on the command line still
wins; the required I/O paths (``-o``, ``-t``, ``--genomes``) stay on the command line.

The file is TOML. Keys are the command's long option names (hyphens or underscores both work), and
values are native TOML scalars/arrays::
