...
from zombi2.species._caps import active_forward_features, species_caps
...
def simulate_species_tree(
    ...
    *,
    ...
    bad = active_forward_features(model, caps)
    if bad:
        raise ValueError(f"{', '.join(bad)} are forward-only; use direction='forward'")
