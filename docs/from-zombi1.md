...
| `SEED` | `--seed` |
| `SHIFT_SPECIATION_RATE_FREQUENCY`, `NUM_SPECIATION_RATE_CATEGORIES`, `BASE_SPECIATION` (and the `EXTINCTION` trio) | the rate grammar: `--birth "PerLineage(1.0).varying_among('lineages', Drift(LogNormal(0.0, 0.2)))"` for a rate inherited and nudged at each split, `changing_at` for a schedule, `scaled_by(TotalDiversity(cap=N))` for a ceiling. v2 has no *category* count — the drift is continuous |
| `TURNOVER` | **no equivalent.** v2 parameterises `birth` and `death` directly |
| `LINEAGE_PROFILE` | **no direct equivalent.** The nearest thing is `--birth "PerLineage(1.0).scaled_by(TotalDiversity(cap=N))"`, which levels diversity off at a ceiling |
...
| `RATE_FILE` · `SCALE_RATES` | the rate grammar. For **per-family** variation — what these keys were usually for — that is `varying_among('families', …)`: `--loss "PerCopy(0.25).varying_among('families', LogNormal(0.0, 0.5))"` draws one factor per family. From Python, one `Random('families', …)` object shared by several rates gives a family a single tempo across all of them. `changing_at` varies a rate in *time* instead, and `scaled_by` scales it by a value from another level |
...
| `KAPPA` | `--kappa` |
| `ST_RATE_MULTIPLIERS` · `GF_RATE_MULTIPLIERS` | the lineage clock: `--substitution "PerSite(1.0).varying_among('lineages', LogNormal(0.0, 0.3))"` (uncorrelated) or `varying_among('lineages', Drift(LogNormal(0.0, 0.3)))` (autocorrelated) |
