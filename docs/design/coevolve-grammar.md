...
**Identity requirements (the hard gate):**
- **Byte-identity:** neutral runs and the zero-burst reductions (`test_codiv_reduces_to_genes_species_when_no_burst`, `test_classe_default_no_cladogenesis_matches_phase1`) produce identical newick+tsv before/after.
- **Statistical-identity:** coupled runs at fixed seed produce identical trees/matrices (`test_*_reproducible` across every edge). The many-σ oracles (ClaSSE jump ~Normal(0,`jump_sigma2`), honest-null suite) must stay green unchanged.
- **RNG draw order** must be replicated exactly when re-expressing any path on the Modifier/co-integrator layer (same constraint the LineageRates port honoured).
...
2. **Byte/behaviour identity via RNG draw order.** The feedback co-integrator (OU step then n uniforms per step), `gene_diversification` (lineage-then-event-then-target weighting), SSE thinning, and `SequenceEvolution`'s `sorted(_natkey)` family draws each have a specific stream. A generic engine changes the stream unless replicated exactly — breaking every seeded test.
