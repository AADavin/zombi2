...
> **Reference:** see the [DNA substitution](../models/dna-substitution.md),
> [protein substitution](../models/protein-substitution.md) and
> [relaxed clocks](../models/relaxed-clocks.md) catalog pages.
...
A **relaxed molecular clock** gives every branch its own substitution rate, so a branch's length in
substitutions is `rate × time` — exactly what turns a chronogram into a phylogram. ZOMBI2 ships a
whole family of clocks in the `zombi2.sequences` namespace, all sharing one `Clock` interface
(`scale(tree, seed=...)` returns the phylogram):

| Family | Clocks | How a branch's rate is set |
|---|---|---|
| **Strict** | `StrictClock` | one rate everywhere |
| **Uncorrelated** | `UncorrelatedLogNormalClock`, `UncorrelatedGammaClock`, `WhiteNoiseClock` | each branch draws an i.i.d. rate |
| **Autocorrelated** | `AutocorrelatedLogNormalClock`, `CIRClock`, `RateVariation` | a branch's rate is anchored to its parent's |

They are all driven the same way, and the [relaxed clocks](../models/relaxed-clocks.md) catalog page
lists them with their parameters. The rest of this section works through **`RateVariation`** — the
discrete-bin, Markov-switching clock from the GTDB archaea study — as the concrete example; the
