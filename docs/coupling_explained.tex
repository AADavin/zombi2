...
\title{\vspace{-1.5cm}{\Large\textcolor{mainblue}{\textsc{How the gene-family coupling model works}}}\\[0.2em]
  {\normalsize A coupling model of non-independent gene families in ZOMBI2}}
...
\emph{together}, the signal that phylogenetic-profiling and direct-coupling (DCA) methods exploit.
...
Fix a panel of $N$ gene families. Inside one genome, their presence/absence is a binary
...
model, \texttt{CoupledRates}, and \textbf{nothing else changes}---not the simulator, the
...
stationary distribution}---this is an \emph{approximate} coupling generator by design (a lost
