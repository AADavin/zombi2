...
conditioning: a conditioned run writes a `conditioned_on` file naming the levels it depends on,
through a rate or `--transfer-to`, so re-running the driver afterwards refuses rather than leaving
the target silently stale. It works **within one run directory** — a driver and a target written to
two directories with `--from` are not linked.
