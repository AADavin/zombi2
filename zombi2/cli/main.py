...
Each subcommand lives in its own module (species, genomes, trait, sequence, coevolve, tools,
...
from zombi2.cli import coevolve, experimental, genomes, sequence, species, tools, trait
...
_RUN = {
    "species": species.run, "genomes": genomes.run, "trait": trait.run,
    "sequence": sequence.run, "coevolve": coevolve.run, "tools": tools.run,
...
def main(argv: list[str] | None = None) -> int:
    ...
    _add_subcommand(
        ...
        "zombi2 trait -t FILE -o DIR [--model MODEL] [options]", trait._add_trait_args,
        epilog=_examples(
