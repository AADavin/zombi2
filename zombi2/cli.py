...
Analysis tools
  tools                compute on ZOMBI2 outputs (reconcile: ALE likelihood; red: RED)
...
def _add_tools_args(p: argparse.ArgumentParser) -> None:
    ...
    layer). Each tool is its own sub-subcommand: ``reconcile`` (ALElite) and ``red`` (RED)."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    _add_subcommand(
        sub, "tools", "compute on ZOMBI2 outputs (reconcile: ALE likelihood; red: RED)",
