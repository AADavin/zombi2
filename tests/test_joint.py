...
The joint half of the one mechanism: `.scaled_by("trait", mapping)` with the live level name
...
def test_joint_result_carries_both_levels():
    ...
    assert set(res.trait.values_by_id) == set(res.complete_tree.extant_leaves())
...
def test_joint_refuses_per_lineage_rate_variation():
    ...
    than merely a gap. It used to be accepted (SPEC §5's rejection rule was applied to the inherited
    and driven kinds but not to this one)."""
    import pytest
...
def test_joint_refuses_a_modifier_it_does_not_thread():
    """The gate was a negative list — it named the inherited and driven kinds and let everything else
    through — where every other level declares what it takes. A per-family draw was the one that
    slipped: accepted, then returning its default factor of 1.0, so the run was quietly not the model
    asked for. The clock and standing diversity were never the problem; the loop threads both and
    steps at their breakpoints, which is why they are in `IMPLEMENTED_MODIFIERS` rather than refused
    alongside."""
    import pytest
