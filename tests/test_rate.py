...
def test_scope_times_modifier():
    r = scope.PerLineage(0.25) * mod.OnTotalDiversity(cap=100)
    assert isinstance(r, Rate)
...
def test_number_on_either_side():
    a = 2.0 * mod.OnTotalDiversity(cap=100)
    b = mod.OnTotalDiversity(cap=100) * 2.0
...
def test_effective_scope_times_modifiers():
    # PerLineage(0.25).total(lineages=4)=1.0 ; OnTotalDiversity(100).factor(diversity=50)=0.5 -> 0.5
    r = scope.PerLineage(0.25) * mod.OnTotalDiversity(cap=100)
    assert r.effective(lineages=4, diversity=50) == pytest.approx(0.5)


def test_effective_with_default_scope_and_time():
    # base 1.0 per lineage: total(lineages=3)=3 ; OnTime factor at t=4 is 0.3 -> 0.9
    r = (1.0 * mod.OnTime({0: 1.0, 3: 0.3})).with_default_scope(scope.PerLineage)
...
def test_effective_requires_scope():
    with pytest.raises(ValueError):
        (1.0 * mod.OnTime({0: 1.0})).effective(time=1.0)  # scope never resolved
...
def test_as_rate_modifier_gets_unit_base_and_default_scope():
    r = as_rate(mod.OnTotalDiversity(cap=100), default_scope=scope.PerLineage)
    assert r.scope == scope.PerLineage(1.0)
    assert r.modifiers == (mod.OnTotalDiversity(cap=100),)


def test_as_rate_existing_rate_resolved():
    r = as_rate(1.0 * mod.OnTime({0: 1.0}), default_scope=scope.PerLineage)
    assert r.scope == scope.PerLineage(1.0)
...
def test_with_default_scope_is_noop_when_scope_set():
    r = (scope.Global(2.0) * mod.OnTotalDiversity(cap=100)).with_default_scope(scope.PerLineage)
...
def test_rate_is_frozen():
    r = 1.0 * mod.OnTotalDiversity(cap=100)
    with pytest.raises(Exception):
        r.base = 2.0  # type: ignore[misc]


def test_rate_next_change_is_earliest_breakpoint():
    r = 1.0 * mod.OnTime({0: 1.0, 5: 0.2}) * mod.OnTotalDiversity(cap=100)
    assert r.next_change(0.0) == 5
    assert r.next_change(5.0) == float("inf")
    assert (1.0 * mod.OnTotalDiversity(cap=100)).next_change(0.0) == float("inf")  # no time-varying part
