...
def test_max_family_size_is_still_refused(tree):
    """Left refused deliberately: a quota is not a weight — it has to decide what happens to a
    segment that is only partly over the cap, and refusing the whole run and refusing part of it are
    different processes."""
    with pytest.raises(ValueError, match="max_family_size is wired at the family resolution"):
        genomes.simulate_genomes_ordered(tree, duplication=0.3, max_family_size=5, seed=1)
