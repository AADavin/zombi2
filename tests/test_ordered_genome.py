...
def test_ordered_genome_runs_with_rearrangements():
    ...
    g = simulate_genomes(tree, rates, initial_size=15, seed=2, genome_factory=_ordered(0.5))
...
def test_ordered_gene_trees_reconstruct():
    ...
    rates = UniformRates(duplication=0.2, transfer=0.1, loss=0.2, origination=0.5,
                         inversion=0.3, transposition=0.3)
    g = simulate_genomes(tree, rates, initial_size=12, seed=3, genome_factory=_ordered(0.6))
...
def test_inversion_conserves_content_and_flips_orientation():
    ...
    g = simulate_genomes(tree, rates, initial_size=8, seed=2, genome_factory=_ordered(0.7))
...
def test_ordered_reproducible():
    ...
    a = simulate_genomes(tree, rates, initial_size=8, seed=6, genome_factory=_ordered(0.5))
    b = simulate_genomes(tree, rates, initial_size=8, seed=6, genome_factory=_ordered(0.5))
...
def test_unordered_genome_ignores_rearrangement_rates():
    ...
    g = simulate_genomes(tree, rates, initial_size=8, seed=2)  # default UnorderedGenome
