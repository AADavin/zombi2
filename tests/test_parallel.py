...
from zombi2 import BirthDeath, UniformRates, run_replicates
...
def test_run_replicates_serial_writes_outputs(tmp_path):
    rows = run_replicates(
        ...
        initial_size=8, seed=1, processes=1,
    )
    assert len(rows) == 4
...
def test_replicates_are_independent():
    ...
    rows = run_replicates(
        ...
        initial_size=10, seed=7, processes=1,
    )
...
def test_reproducible_by_base_seed(tmp_path):
    kw = dict(n_tips=8, age=4.0, duplication=0.15, transfer=0.1, loss=0.2,
              origination=0.5, initial_size=8, seed=42, processes=1)
...
def test_parallel_matches_serial(tmp_path):
    ...
    kw = dict(species_model=BirthDeath(1.0, 0.3), n_tips=10, age=4.0,
              rates=UniformRates(0.2, 0.1, 0.2, 0.5), initial_size=10, seed=5)
