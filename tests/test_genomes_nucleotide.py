...
def _run(seed=1, inversion=2, chromosomes=1, **kw):
    ...
    params = dict(inversion=inversion, inversion_extent=15, root_length=120, chromosomes=chromosomes,
                  seed=seed)
    params.update(kw)
...
def test_every_node_has_a_karyotype_of_conserved_length():
    ...
    assert all(g.length == 300 for g in r.node_genomes.values())  # 3 x 100, inversion conserves length
    assert all(len(g.chromosomes) == 3 for g in r.node_genomes.values())   # no tier yet: count conserved
...
def test_every_node_carries_the_whole_initial_sequence_permuted():
    ...
    r = simulate_genomes_nucleotide(sp, inversion=3, inversion_extent=10, chromosomes=specs, seed=3)
...
def test_zero_inversion_leaves_each_replicon_a_single_block():
    ...
    for g in r.node_genomes.values():
...
def test_inversions_recorded_within_their_branch_and_chromosome():
    ...
    assert r.rearrangements
    node_chroms = {n: {c.id for c in r.node_genomes[n].chromosomes} for n in r.node_genomes}
...
def test_deterministic_given_seed():
    ...
    kw = dict(inversion=2.5, inversion_extent=15, chromosomes=[(120, "circular"), (30, "linear")], seed=8)
    a = simulate_genomes_nucleotide(sp, **kw)
    b = simulate_genomes_nucleotide(sp, **kw)
    assert all(a.mosaic(n) == b.mosaic(n) for n in a.node_genomes)
...
def test_simulate_validation():
    ...
    with pytest.raises(ValueError):
        simulate_genomes_nucleotide(sp, inversion_extent=0)
    with pytest.raises(ValueError):
    ...
    with pytest.raises(ValueError):
        simulate_genomes_nucleotide(sp, translocation_extent=0)
    with pytest.raises(ValueError):
        simulate_genomes_nucleotide(sp, inversion_probability=2.0)
    with pytest.raises(ValueError):
        simulate_genomes_nucleotide(sp, loss=-1.0)
    with pytest.raises(ValueError):
        simulate_genomes_nucleotide(sp, loss_extent=0)
    with pytest.raises(ValueError):
        simulate_genomes_nucleotide(sp, duplication=-1.0)
    with pytest.raises(ValueError):
        simulate_genomes_nucleotide(sp, duplication_extent=0)
...
def test_duplication_keeps_all_ancestry_with_extra_copies():
    ...
    r = simulate_genomes_nucleotide(sp, duplication=2, duplication_extent=10, inversion=1,
    ...
    assert any(len(r.ancestry(n)) > len(full) for n in r.node_genomes)   # copies really exist
...
def test_loss_weakens_the_invariant_monotonically():
    ...
    sp = simulate_species_tree(birth=1.0, death=0.3, n_extant=10, seed=3)
    r = simulate_genomes_nucleotide(sp, loss=2.5, loss_extent=10, inversion=1,
    ...
    assert any(set(r.ancestry(n)) < full for n in r.node_genomes)   # something really was lost
...
def _tier(seed, **kw):
    sp = simulate_species_tree(birth=1.0, death=0.3, n_extant=10, seed=seed)
    params = dict(inversion=1, inversion_extent=12, fission=0.6, fusion=0.6,
...
def test_tier_changes_the_chromosome_number():
    _sp, r = _tier(seed=5)
    assert len({len(g.chromosomes) for g in r.node_genomes.values()}) > 1
...
def test_translocation_conserves_the_chromosome_count():
    ...
    r = simulate_genomes_nucleotide(sp, translocation=3, translocation_extent=10,
                                    chromosomes=_TIER_SPECS, seed=5)
    assert all(len(g.chromosomes) == 3 for g in r.node_genomes.values())   # material moves, count does not
...
def test_strong_invariant_survives_translocation():
    ...
    for seed in range(3):
        sp = simulate_species_tree(birth=1.0, death=0.3, n_extant=10, seed=seed)
        r = simulate_genomes_nucleotide(sp, translocation=4, translocation_extent=8, inversion=1,
...
def test_all_four_events_compose():
    ...
    full = sorted((s, p) for s, (length, _t) in enumerate(_TIER_SPECS) for p in range(length))
    assert all(r.ancestry(node_id) == full for node_id in r.node_genomes)
...
def test_copy_number_matches_the_genealogy_log():
    ...
    r = simulate_genomes_nucleotide(sp, duplication=2.5, loss=2.5, duplication_extent=8,
                                    loss_extent=8, inversion=1.5, translocation=1.5, fission=0.15,
    ...
    for node_id in r.node_genomes:
        genome = Counter((s, p) for chrom in r.node_genomes[node_id].chromosomes
...
def test_every_copy_lineage_is_born_and_traces_to_a_root():
    ...
    present = {b.copy for g in r.node_genomes.values() for chrom in g.chromosomes for b in chrom.blocks}
...
def test_speciation_re_mints_every_copy_lineage():
    ...
    tree = r.complete_tree
    for node_id, g in r.node_genomes.items():
        node = tree.nodes[node_id]
        if not node.children:
            continue
        parent_copies = {b.copy for chrom in g.chromosomes for b in chrom.blocks}
        for c in node.children:
            child_copies = {b.copy for chrom in r.node_genomes[c].chromosomes for b in chrom.blocks}
...
def test_recovery_ancestry_neutral_is_the_species_tree():
    ...
    leaves = set(r.complete_tree.extant_leaves())
...
def test_duplication_shows_up_as_a_branch_in_a_block_tree():
    ...
    r = simulate_genomes_nucleotide(sp, duplication=3, duplication_extent=10,
...
def test_transfer_is_additive_keeps_all_ancestry_and_grows_copies():
    ...
    r = simulate_genomes_nucleotide(sp, transfer=3.5, transfer_extent=10, inversion=1,
    ...
    assert any(len(r.ancestry(n)) > len(_XFER_FULL) for n in r.node_genomes)   # extra copies exist


def test_transfer_recipient_is_a_distinct_contemporaneous_lineage():
    sp = simulate_species_tree(birth=1.0, death=0.3, n_extant=10, seed=4)
    r = simulate_genomes_nucleotide(sp, transfer=4, transfer_extent=8, chromosomes=_XFER_SPECS, seed=4)
...
def test_recovery_cross_check_holds_with_transfer():
    ...
    for seed in range(4):
        sp = simulate_species_tree(birth=1.0, death=0.3, n_extant=8, seed=seed)
        r = simulate_genomes_nucleotide(sp, transfer=2.5, transfer_extent=8, duplication=1.5,
...
def test_transfer_is_a_horizontal_edge_in_a_block_tree():
    ...
    r = simulate_genomes_nucleotide(sp, transfer=4, transfer_extent=10, chromosomes=_XFER_SPECS, seed=3)
...
def test_transfer_to_distance_and_self_transfer_run():
    ...
    for kw in (dict(transfer_to="distance"), dict(transfer_to=Distance(decay=2.0)),
        ...
        r = simulate_genomes_nucleotide(sp, transfer=3, transfer_extent=8, chromosomes=_XFER_SPECS,
                                        seed=5, **kw)
        assert any(isinstance(e, Transfer) for e in r.events)
        for node_id in r.node_genomes:                              # additive: full ancestry preserved
...
def test_clades_steer_a_nucleotide_transfer_between_two_clades():
    ...
    for node_id in r.node_genomes:                                  # still additive: nothing lost
        assert set(r.ancestry(node_id)) == _XFER_FULL
...
def test_a_kernel_that_lets_nobody_receive_fires_no_nucleotide_transfer():
    ...
    assert all(sorted(blocked.ancestry(n)) == sorted(_XFER_FULL) for n in blocked.node_genomes)
...
def test_transposition_is_ancestry_neutral_and_conserves_counts():
    ...
    for seed in range(3):
        sp = simulate_species_tree(birth=1.0, death=0.3, n_extant=10, seed=seed)
        r = simulate_genomes_nucleotide(sp, transposition=4, transposition_extent=8,
        ...
        for node_id in r.node_genomes:
            ...
            assert len(r.node_genomes[node_id].chromosomes) == 2    # ...and the chromosome count
...
def test_transposition_works_within_a_single_chromosome():
    ...
    sp = simulate_species_tree(birth=1.0, death=0.2, n_extant=6, seed=2)
    r = simulate_genomes_nucleotide(sp, transposition=5, transposition_extent=8, chromosomes=1,
                                    root_length=100, seed=2)
    assert any(isinstance(x, Transposition) for x in r.rearrangements)
    full = sorted((0, p) for p in range(100))
    assert all(r.ancestry(n) == full for n in r.node_genomes)       # single replicon, ancestry conserved
...
def test_origination_adds_de_novo_sources_beyond_the_root():
    ...
    r = simulate_genomes_nucleotide(sp, origination=0.5, origination_extent=10, chromosomes=specs, seed=3)
    ...
    assert all(root_full <= set(r.ancestry(n)) for n in r.node_genomes)
    assert any(any(s >= len(specs) for (s, _p) in r.ancestry(n)) for n in r.node_genomes)
...
def test_origination_mints_a_gene():
    ...
    r = simulate_genomes_nucleotide(sp, origination=0.5, origination_extent=10, chromosomes=specs, seed=6)
    denovo = [e for e in r.events if isinstance(e, Origination) and e.source >= len(specs)]
    assert denovo
...
def test_origination_family_roots_at_its_own_branch():
    ...
    specs = [(80, "circular"), (30, "linear")]
    sp = simulate_species_tree(birth=1.0, death=0.2, n_extant=8, seed=6)
    r = simulate_genomes_nucleotide(sp, origination=0.5, origination_extent=10, inversion=1,
...
def test_recovery_cross_check_holds_with_origination():
    ...
    for seed in range(3):
        sp = simulate_species_tree(birth=1.0, death=0.3, n_extant=8, seed=seed)
        r = simulate_genomes_nucleotide(sp, origination=0.4, origination_extent=10, loss=1.5,
...
def test_transposition_and_origination_validation():
    sp = simulate_species_tree(birth=1.0, death=0.3, n_extant=4, seed=1)
    for kw in (dict(transposition=-0.1), dict(transposition=5, transposition_extent=0),
               dict(origination=-0.1), dict(origination=0.1, origination_extent=0)):
        with pytest.raises(ValueError):
...
def test_chromosome_origination_adds_a_replicon_carrying_a_gene():
    ...
    assert all(c.length > 0 and c.n_genes >= 1
               for g in r.node_genomes.values() for c in g.chromosomes if c.id in de_novo)
    for node_id in r.node_genomes:
        assert root_full <= set(r.ancestry(node_id))               # the seed material is untouched...
    assert any(any(s >= len(specs) for (s, _p) in r.ancestry(n)) for n in r.node_genomes)  # ...plus new
    assert any(len(g.chromosomes) > len(specs) for g in r.node_genomes.values())   # the count grew
...
def test_chromosome_loss_kills_whole_chromosomes_as_a_subset():
    ...
    assert all(len(g.chromosomes) >= 1 for g in r.node_genomes.values())  # never the last chromosome
    tree = r.complete_tree
    ...
    assert any(len(r.ancestry(n)) < len(full) for n in r.node_genomes)  # some material really was lost
...
def test_seeding_lays_down_the_alternating_chain():
    ...
    root = r.node_genomes[r.complete_tree.root].chromosomes[0]
...
def test_no_genes_declared_is_the_uniform_model():
    ...
    assert all(not b.is_gene for g in r.node_genomes.values() for c in g.chromosomes for b in c.blocks)
...
def test_genes_are_never_split_under_every_event():
    for seed in range(2):
        ...
        r = simulate_genomes_nucleotide(
            ...
            transfer=1.5, fission=0.1, fusion=0.1, inversion_probability=0.4, inversion_extent=20,
            loss_extent=20, duplication_extent=20, transfer_extent=20, seed=seed, **_GENIC)
        spans = _gene_spans(r)
...
def test_declared_genes_come_back_as_intact_root_blocks():
    ...
    r = simulate_genomes_nucleotide(sp, inversion=2.5, transposition=1.5, loss=2, duplication=2,
                                    inversion_extent=30, loss_extent=30, seed=3, **_GENIC)
    root_genes = {(b.source, b.start, b.end) for chrom in r.node_genomes[r.complete_tree.root].chromosomes
...
def test_gene_trees_are_one_per_gene_not_per_root_block():
    ...
    r = simulate_genomes_nucleotide(sp, inversion=2.5, transposition=1.5, loss=2.0, duplication=2.0,
                                    inversion_extent=30, loss_extent=30, duplication_extent=30,
                                    seed=3, **_GENIC)
...
def test_seeding_from_a_gff(tmp_path):
    ...
    chroms = r.node_genomes[r.complete_tree.root].chromosomes
...
def test_gff_genes_are_never_split(tmp_path):
    ...
    r = simulate_genomes_nucleotide(sp, gff=_gff(tmp_path), inversion=2.0, transposition=1.0,
                                    ...
                                    inversion_extent=100, loss_extent=100, duplication_extent=100,
                                    transfer_extent=100, inversion_probability=0.4, seed=2)
    spans = _gene_spans(r)
    assert spans
    for fam, seen in spans.items():
        assert len(seen) == 1, f"gene family {fam} was cut into {seen}"
...
def test_gff_genes_get_one_tree_each_lookupable_by_name(tmp_path):
    ...
    r = simulate_genomes_nucleotide(sp, gff=_gff(tmp_path), inversion=2.0, loss=1.5, duplication=1.5,
                                    inversion_extent=100, loss_extent=100, duplication_extent=100, seed=3)
    ...
    for extent in (200, 20_000):                     # short arcs and arcs spanning many genes
        ...
        for seed in range(12):
            r = simulate_genomes_nucleotide(sp, genes=100, gene_length=94, chromosomes=1,
                                            root_length=10_000, loss=3.0, loss_extent=extent, seed=seed)
...
def test_with_no_events_the_genome_is_exactly_what_was_declared(tmp_path):
    ...
    for lid in (n.id for n in (r.complete_tree.nodes[_i] for _i in r.complete_tree.extant_leaves())):
        chrom1, plasmid = r.node_genomes[lid].chromosomes
...
def test_the_even_layout_also_round_trips():
    ...
    for lid in (n.id for n in (r.complete_tree.nodes[_i] for _i in r.complete_tree.extant_leaves())):
        chrom, = r.node_genomes[lid].chromosomes
...
def test_inversions_along_the_tree_match_an_independent_replay():
    ...
    r = simulate_genomes_nucleotide(sp, genes=n_genes, gene_length=gene_len, chromosomes=1,
                                    root_length=length, inversion=25.0, inversion_extent=400, seed=4)
    ...
    while stack:
        ...
        assert r.node_genomes[nid].chromosomes[0].trace_back() == arr, f"node {nid} diverged from the replay"
        if tree.nodes[nid].children:
            for c in tree.nodes[nid].children:
                stack.append((c, arr))

    full = sorted((0, i) for i in range(length))
    for leaf in (tree.nodes[_i] for _i in tree.extant_leaves()):                               # nothing gained, nothing lost
        assert r.ancestry(leaf.id) == full
        seen = {b.gene for b in r.node_genomes[leaf.id].chromosomes[0].blocks if b.is_gene}
...
def test_two_chromosomes_ancestry_neutral_tier_conserves_everything():
    ...
    sp = simulate_species_tree(birth=1.0, death=0.0, n_extant=5, seed=1)
    r = simulate_genomes_nucleotide(
        sp, inversion=6.0, inversion_extent=300, translocation=6.0, translocation_extent=300,
        transposition=3.0, transposition_extent=300, fission=3.0, fusion=3.0, seed=5, **_TWO)
    ...
    for node_id in r.node_genomes:                                  # nothing gained, nothing lost, anywhere
        assert r.ancestry(node_id) == _TWO_FULL

    karyotypes = {len(r.node_genomes[leaf.id].chromosomes) for leaf in (r.complete_tree.nodes[_i] for _i in r.complete_tree.extant_leaves())}
    ...
    assert any(len({b.source for b in c.blocks}) == 2
               for leaf in (r.complete_tree.nodes[_i] for _i in r.complete_tree.extant_leaves())
               for c in r.node_genomes[leaf.id].chromosomes)

    spans = _gene_spans(r)
    assert len(spans) == 16                                    # 8 genes on each replicon
    for fam, seen in spans.items():
        assert seen == {r.gene_spans[fam]}                     # every gene whole, where it was declared
    for leaf in (r.complete_tree.nodes[_i] for _i in r.complete_tree.extant_leaves()):                      # ...and every gene still present
        present = {b.gene for c in r.node_genomes[leaf.id].chromosomes for b in c.blocks if b.is_gene}
...
def test_two_chromosomes_with_the_whole_event_set():
    ...
    r = simulate_genomes_nucleotide(
        ...
        loss=3.0, loss_extent=250, duplication=3.0, duplication_extent=250,
    ...
    assert all(c.n_genes >= 1 for g in r.node_genomes.values() for c in g.chromosomes)
    ...
    leaves = list(r.complete_tree.extant_leaves())          # the cross-check, per gene
...
def test_extinct_lineages_evolve_donate_and_are_pruned():
    ...
    *donated* to a surviving lineage does — the classic transfer-from-the-dead."""
    ...
    r = simulate_genomes_nucleotide(sp, chromosomes=[(1500, "circular")], genes=10, gene_length=100,
                                    inversion=3.0, inversion_extent=200, loss=3.0, loss_extent=200,
                                    duplication=3.0, duplication_extent=200,
                                    transfer=6.0, transfer_extent=200, seed=4)

    # the dead still evolved, and we kept their genomes
    assert set(r.node_genomes) == set(tree.nodes)
    assert all(r.node_genomes[d].length > 0 for d in dead)
    ...
    in_extant = {b.gene for lid in extant for c in r.node_genomes[lid].chromosomes
                 for b in c.blocks if b.is_gene}
    in_dead = {b.gene for d in dead for c in r.node_genomes[d].chromosomes for b in c.blocks if b.is_gene}
...
_HOT = dict(inversion=8.0, inversion_extent=200, translocation=4.0, translocation_extent=200,
            transposition=4.0, transposition_extent=200, loss=6.0, loss_extent=200,
            duplication=6.0, duplication_extent=200, transfer=4.0, transfer_extent=200,
            fission=2.0, fusion=2.0, chromosome_origination=1.0, chromosome_loss=0.5,
            origination=1.0, origination_extent=50)
...
def _invariants_hold(r):
    ...
    extant = list(r.complete_tree.extant_leaves())
    for fam, gt in r.gene_trees.items():
        s, a, b = r.gene_spans[fam]
        recovered = collections.Counter(t.species for t in (_tips(gt.extant) if gt.extant else [])
                                        if t.kind == "extant")
...
def test_degenerate_genomes_survive_every_event(label, kwargs):
    ...
    assert all(g.length >= 1 for g in r.node_genomes.values())      # a genome is never wiped out entirely
    _invariants_hold(r)
...
def test_extents_far_larger_than_the_genome():
    ...
    r = simulate_genomes_nucleotide(sp, chromosomes=1, root_length=500, genes=4, gene_length=50,
                                    inversion=8.0, inversion_extent=10**6,
                                    loss=6.0, loss_extent=10**6,
                                    duplication=6.0, duplication_extent=10**6, seed=1)
    assert all(g.length >= 1 for g in r.node_genomes.values())
    _invariants_hold(r)
...
def test_a_genome_packed_with_genes_needs_no_gap_between_them():
    ...
    r = simulate_genomes_nucleotide(sp, root_length=1000, genes=10, gene_length=100, inversion=8.0,
                                    inversion_extent=200, duplication=8.0, loss=8.0, seed=1)
    ...
    assert all(g.length % 100 == 0 for g in r.node_genomes.values())
...
def test_a_de_novo_replicon_is_born_with_a_gene_and_can_grow():
    ...
    r = simulate_genomes_nucleotide(sp, chromosomes=[(800, "circular")], genes=4, gene_length=80,
                                    ...
                                    translocation_extent=150, transfer=4.0, transfer_extent=150,
                                    origination=2.0, origination_extent=60, inversion=2.0,
                                    inversion_extent=150, seed=3)
    ...
    de_novo = {c for e in r.chromosome_events if e.kind == "origination" and e.time > 0.0
               for c in e.children}
    assert de_novo
    born = {c.id for g in r.node_genomes.values() for c in g.chromosomes if c.id in de_novo}
...
def test_no_chromosome_is_ever_left_without_a_gene():
    ...
    for seed in range(4):
        ...
        r = simulate_genomes_nucleotide(
            ...
            loss=12.0, loss_extent=400,                  # brutal loss: tries hard to empty things
            translocation=8.0, translocation_extent=400, # tries to carry the last gene away
            fission=6.0, fusion=3.0,                     # tries to split off a geneless half
            transposition=4.0, transposition_extent=200, inversion=4.0, inversion_extent=200,
            duplication=3.0, duplication_extent=200, transfer=3.0, transfer_extent=200,
            chromosome_origination=2.0, chromosome_loss=1.0, origination_extent=80, seed=seed)
        for node_id, g in r.node_genomes.items():
...
def test_written_blocks_tile_every_chromosome_of_every_node(tmp_path):
    ...
    for (node, chrom_id), chrom_rows in by_chromosome.items():
        seen.add(node)
        chrom = next(c for c in r.node_genomes[node].chromosomes if c.id == chrom_id)
        at = 0
    ...
    assert seen == set(r.node_genomes), "ancestors as well as tips must be written"
...
def test_assembly_tiles_every_node_exactly_as_its_trace_back():
    ...
    g = simulate_genomes_nucleotide(sp, inversion=3.0, inversion_extent=80, loss=0.4, loss_extent=40,
                                    duplication=0.4, duplication_extent=40, transfer=0.6,
                                    transfer_extent=60, root_length=600, genes=3, gene_length=90,
                                    seed=4)
    for node_id in sorted(g.node_genomes):                    # no node is skipped and none refuses
        assert _expand(g, node_id) == g.trace_back(node_id)
    assert len(g.node_genomes) > len(g.complete_tree.extant_leaves()), "the tree has no ancestors to check"
...
def test_the_partition_is_at_least_as_fine_as_every_nodes_blocks():
    ...
    sp = simulate_species_tree(birth=1.0, death=0.3, n_extant=8, seed=4)
    g = simulate_genomes_nucleotide(sp, inversion=3.0, inversion_extent=80, loss=1.5, loss_extent=60,
                                    duplication=0.6, duplication_extent=40, transfer=0.6,
                                    transfer_extent=60, root_length=600, genes=3, gene_length=90,
                                    seed=4)
...
def _export_run(tmp_path, **kw):
    ...
    params = dict(root_length=1200, genes=4, gene_length=150, inversion=3.0, inversion_extent=200,
...
def test_gff_and_bed_are_written_for_every_genome_and_name_the_fasta_records(tmp_path):
    ...
    labels = [node_label(i) for i in g.node_genomes] + ["initial"]
...
def test_an_unwired_modifier_is_still_refused_cleanly():
    ...
    assert simulate_genomes_nucleotide(sp, root_length=200, inversion=2.0, seed=1).node_genomes
...
def test_an_insertion_is_a_root_carrying_spacer_rather_than_a_gene():
    ...
    inserts = [e for e in g.events if isinstance(e, Origination) and e.insertion]
    assert inserts
    for e in inserts:
        assert e.start == 0 and e.end > 0        # a fresh source, its own coordinate space
        assert not e.initial
...
def test_insertion_origination_and_the_initial_genome_are_counted_apart():
    ...
    assert s["insertion"] == sum(1 for e in g.events
                                 if isinstance(e, Origination) and e.insertion)
    assert s["origination"] == sum(1 for e in g.events if isinstance(e, Origination)
                                   and not e.initial and not e.insertion)
