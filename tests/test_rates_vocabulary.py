...
from zombi2.rates import Drawn, Inherited, LogNormal, ScaledBy, SetBy, Time, Weights
...
class TestTheGridBuildsWhatTheEnginesRun:
    ...
    def test_a_drawn_cell_reports_its_kind_and_unit(self):
        for unit in ("family", "lineage", "chromosome", "site"):
            assert Drawn(per=unit, dist=LogNormal(0.0, 0.5)).reads == ("drawn", unit)

    def test_an_inherited_cell_reports_its_kind_and_unit(self):
        assert Inherited(per="lineage", dist=LogNormal(0.0, 0.2)).reads == ("inherited", "lineage")
    ...
    def test_the_distribution_is_what_makes_two_cells_the_same(self):
        """A cell is its unit and its distribution — nothing about how either was typed."""
        assert Drawn(per="family", dist=LogNormal(0.0, 0.5)) == Drawn(per="family", dist=LogNormal(0.0, 0.5))
        assert Drawn(per="family", dist=LogNormal(0.0, 0.5)) != Drawn(per="family", dist=LogNormal(0.0, 0.6))
        assert Drawn(per="family", dist=LogNormal(0.0, 0.5)) != Drawn(per="lineage", dist=LogNormal(0.0, 0.5))

    def test_a_cell_without_a_distribution_is_refused(self):
...
class TestACellNobodyBuiltRefusesByName:
    ...
    def test_a_cell_nobody_built_constructs_and_the_engine_refuses_it(self):
        ...
        cell = Drawn(per="chromosome", dist=LogNormal(0.0, 0.5))
    ...
    def test_a_cell_the_wrong_level_carries_is_named_by_its_cell(self):
        ...
        with pytest.raises(ValueError, match="drawn per lineage"):
            genomes.simulate_genomes_family(
                ...
                loss=0.2 * Drawn(per="lineage", dist=LogNormal(0.0, 0.5)), initial_families=3, seed=1)

    def test_a_unit_that_is_not_a_unit_lists_the_units(self):
        with pytest.raises(ValueError, match="unknown unit 'banana'"):
            Drawn(per="banana", dist=LogNormal(0.0, 0.5))
...
class TestTheVerbsAreForValuesNotFactors:
    ...
    def test_wrapping_a_draw_in_a_verb_is_refused(self):
        ...
        with pytest.raises(TypeError, match="already a factor"):
            ScaledBy(Drawn(per="family", dist=LogNormal(0.0, 0.5)))
...
def test_one_object_read_twice_is_still_one_draw_in_the_new_spelling():
    ...
    speed = Drawn(per="family", dist=LogNormal(0.0, 0.6))
    ...
    apart = genomes.simulate_genomes_family(
        tree, duplication=0.3 * Drawn(per="family", dist=LogNormal(0.0, 0.6)),
        loss=0.3 * Drawn(per="family", dist=LogNormal(0.0, 0.6)), initial_families=20, seed=2)
