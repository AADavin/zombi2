...
def test_a_gene_can_drive_a_trait():
    ...
    driven = simulate_discrete(tree, switch=PerLineage(0.1).scaled_by(
...
def test_a_driver_that_never_changes_leaves_the_run_byte_identical():
    ...
    driven = simulate_discrete(tree, switch=PerLineage(0.2).scaled_by(g.presence("tox"), {"present": 1.0}),
                               **kw)
...
def test_completion_drives_a_trait():
    ...
    driven = simulate_discrete(tree, switch=PerLineage(0.05).scaled_by(
...
def test_a_nucleotide_module_completes_the_way_a_family_one_does():
    ...
    simulate_discrete(tree, states=["a", "b"], start="a", seed=2,           # and it drives
                      switch=PerLineage(0.05).scaled_by(g.completion("operon"), Curve(lambda f: 1.0 + f)))
